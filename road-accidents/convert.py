...
# Load data
with open('road-accidents/RoadTrafficAccidentLocations.json', 'r') as f:
    data = json.load(f)
...
# Write types CSVs
def write_type_csv(filename, types_dict):
...
write_type_csv('road-accidents/ccident_types.csv', accident_types)
write_type_csv('road-accidents/everity_categories.csv', severity_categories)
write_type_csv('road-accidents/oad_types.csv', road_types)

# Now main accidents CSV
dropped_count = 0
valid_count = 0
with open('road-accidents/accidents.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow([
        'accident_type_uid', 'severity_category_uid',
        'involving_pedestrian', 'involving_bicycle', 'involving_motorcycle',
        'longitude', 'latitude', 'swiss_e', 'swiss_n',
        'canton_code', 'municipality_code', 'timestamp'
    ])
    for i, feature in enumerate(features):
        if i % 1000 == 0:
        ...
        valid_count += 1
        writer.writerow([
            props['AccidentType'], props['AccidentSeverityCategory'],
            props.get('AccidentInvolvingPedestrian', ''), props.get('AccidentInvolvingBicycle', ''), props.get('AccidentInvolvingMotorcycle', ''),
            longi, lati, swiss_e, swiss_n, canton, muni, timestamp
        ])
